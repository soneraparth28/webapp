...
class AdminController extends Controller
{
    ...
    // creator
		...
		$creators               = User::orderBy('id','DESC')->where('verified_id','yes')->count();
		$sales = Transactions::whereApproved('1')->sum('amount');
		$creator_earning  =  User::where('verified_id', 'yes')->sum('balance');
		$referralTransaction = ReferralTransactions::sum('earnings');
    ...
    $user = User::findOrFail($id);
		...
		//code for plan change
		 $is_extend=false;
		 $plan = $request->plan;
			 if($request->plan=='monthly_extend'){
			 	$plan = "monthly";
			 	$is_extend=true;
			 }
            $plans = Plans::where('user_id', $user->id)->get()->first();
            if($plans){
                if($plan == "no_plan"){
                ...
                if($plan!=$plans->interval || $is_extend){
                    
                if($plan == "monthly"){
                    $plans->name = 'user_'.$user->id.'_monthly';
                    $price = "99";
                    $plans->interval = 'monthly';
                    $interval = 'monthly';
                }elseif($plan == "free"){
                     $plans->name = 'user_'.$user->id.'_free';
                     $price = "0";
                     $plans->interval = 'free';
                     $interval = 'free';
                }else{
                     $plans->name = 'user_'.$user->id.'_yearly';
                     $price = "799";
                ...
                if($interval == 'free') { $end_date_for_revolut = date('Y-m-d', strtotime("+12 month", $current_date_str)); }
	...
	public function transactions(Request $request)
	{
		...
		return view('admin.transactions', ["data" => $data, "transactions" => $this->getMappedTransactions(), "countries" => Countries::all()]);
	}//<--- End Method
	...
	public function salesRefund($id)
  {
    ...
    public function transactionMap(Request $request) {
        $filteredItems =  $this->filterTransactions($this->getMappedTransactions(), $request);

        if(isset($request["format_by_date"]) && $request["format_by_date"]) $filteredItems = $this->mapDataByDate($filteredItems, $request);
        elseif(isset($request["format_country"]) && $request["format_country"]) $filteredItems = $this->mapDataByCountry($filteredItems, (int)$request->start, (int)$request->end);
    ...
    public function lookupTransactionsByFilter($dateStart, $dateEnd, $countryCode) {
         $timeStart = strtotime($dateStart);
         $timeEnd = strtotime($dateEnd);
         if(!$timeStart || !$timeEnd) return $this->lookupPayments();

        $taxId = $countryCode !== "total" ? TaxRates::where("country", $countryCode)->pluck("id")->first() : null;
         $transactions = Transactions::where(function ($query) use ($timeStart, $timeEnd, $taxId) {
             if(!empty($taxId)) $query->where("taxes", $taxId);
             $query->where("created_at", ">=", date("Y-m-d", $timeStart))
                 ->whereDate("created_at", "<", date("Y-m-d", $timeEnd));
         })->orderBy('id','desc')->paginate(20);

        return view('users.my_payments', ['transactions' => $transactions]);
    }


    public function lookupPayments() {
