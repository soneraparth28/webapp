...
class SubscriptionsController extends Controller
{
  ...
  /**
	 ...
	 * @return Response
	 */
  public function buy_throgh_revolut() {
    ...
    //$amount = $amount*100;
    //$amount = self::priceWithTax($amount);
    ...
    $plan = Plans::where(['user_id'=>$creator_id])->first();
  ...
  public function buy()
  {
    ...
    /*$checkSubscription = auth()->user()->mySubscriptions()
      ->whereStripePrice($plan->name)
        ->where('ends_at', '>=', now())->first();*/
    $checkSubscription = auth()->user()->mySubscriptions()
    ->where('subscriptions.stripe_price', $plan->name)
  ...
  $validator = Validator::make($this->request->all(), [
    ...
    public function update_plans(Request $request){
     ...
     public function update_plans_old(Request $request){
...
//echo "<Pre>";print_r($subscriptions);die;
  ...
  // when called first time subscription id will be given explicitly as time of payment subscription was not performed
  public function startRevolutSubscriptionLocally($payment_id, $interval, $user_id, $subscription_id='') {
    ...
    if($old_subscription)
    {
      ...
      $end_date_for_revolut = date('Y-m-d', strtotime("+1 month", $current_date_str));
         ...
         if($interval == 'quarterly') { $end_date_for_revolut = date('Y-m-d', strtotime("+3 month", $current_date_str)); }
         if($interval == 'biannually') { $end_date_for_revolut = date('Y-m-d', strtotime("+6 month", $current_date_str)); }
         if($interval == 'monthly') { $end_date_for_revolut = date('Y-m-d', strtotime("+1 month", $current_date_str)); }
         if($interval == 'yearly') { $end_date_for_revolut = date('Y-m-d', strtotime("+12 month", $current_date_str)); }
        ...
        $subscription->last_payment = null;
        $subscription->free = 'no';
        $subscription->cancelled = 'no';
        $subscription->rebill_wallet = 'no';
        $subscription->taxes = '';
        $subscription->ends_at = $end_date_for_revolut;
        $subscription->subscription_id = $payment_id;
        $subscription->user_id = $subscribe_by;
    ...
    if($subscriptionItem === "cr") {
        ...
        $to_subscribe = $user_id;

        // Find the user to subscribe
        $to_subscribe_user = $subscribe_by_user;

        // Check if Plan exists
        $plans = Plans::where('user_id', $user_id)->get()->first();

        $plans->name = 'user_'.$user_id.'_'.$interval;
        ...
        $subscription->stripe_price = $plans->name;
        $amount = $plans->price;
        $platformFee = $to_subscribe_user->custom_fee;
    ...
    elseif($subscriptionItem === "addon") {
        $to_subscribe = $mollie_subscription_details['addon_id'];
        $subscription->stripe_price = "addon_$to_subscribe";
        $subscription->name = 'addon';
        ...
        $amount = number_format($amount, 2, ".", "");
        $platformFee = 100;
        $subscription->stripe_id = $addon->id;
    }
    else return;
  ...
  private function startRevolutSubscriptionLocally_old($payment_id, $subscription_id='') {
