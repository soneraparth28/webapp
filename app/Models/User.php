...
class User extends Authenticatable implements HasLocalePreference
{
    ...
    public function isTaxable()
    {
      ...
      if(!empty($country_id)){
        $country_code = Countries::where('id', $country_id)->pluck('country_code')->first();
      }
      else{
        $country_code = 'dasdasd';
      }

      return TaxRates::where(['name' => 'VAT', 'country' => $country_code])->limit(1)->get();
    ...
    // public function newIsTaxableArray(){

    //   $country_id = auth()->user()->countries_id;

    //   if(!empty($country_id)){
    //     $country_code = Countries::where('id', $country_id)->pluck('country_code')->first();
    //     return TaxRates::where(['name' => 'VAT', 'country' => $country_code])->first()->toArray();
    //   }

    // }

    public function newIsTaxable(){
      ...
      public function checkSubscription($user)
      {
        ...
        public function totalSubscriptionsActive()
        {
          return $this->mySubscriptions()
            ...
            $query->where('subscriptions.stripe_id', '')
                ->where('subscriptions.ends_at', '>=', now())
                ->orWhere(function ($query) {
                    $query->where('subscriptions.stripe_status', 'active')
                        ->where('subscriptions.stripe_id', '<>', '')
                        ->whereIn('subscriptions.stripe_price', $this->plans()->pluck('plans.name'));
                })
                ->orWhere(function ($query) {
                    $query->where('subscriptions.stripe_id', '')
                        ->where('subscriptions.stripe_price', $this->plan);
                });
        })
        ->where('subscriptions.free', 'yes')
        ->count();
        }
     ...
     */
      ...
      */
      ...
      public function sales()
  		...
  		public function ccRefers($creator_id){
  		    return User::where('creator_referred_id', $creator_id);
  		}
  		
  		public function ccReferralTransactions() {
            return $this->hasMany(ContentCreatorsReferralsTransactions::class, 'user_id');
        }
