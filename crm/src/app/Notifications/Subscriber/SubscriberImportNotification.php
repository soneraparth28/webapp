...
class SubscriberImportNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ->view('notification.mail.subscriber.index', [
                'template' => strtr(
                    $content,
                    (new SubscriberTag($this->brand, $this->auth, $notifiable))->import()
                )
            ])
    ...
    public function toDatabase($notifiable)
    {
        return [
            ...
            "url" => route('tenant.subscribers.lists', ['brand_dashboard' => $this->brand->id]),
