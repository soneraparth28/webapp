...
public static function resizeImageFixed($image,$width,$height,$imageNew = null)
{
    ...
    public static function setTax($value, $isTaxable) {
        if ($isTaxable->count()) {
            $taxes = 0;
            ...
            $valueWithTax = round($taxes * $value / 100, 2);
            $value = ((float)$value + $valueWithTax);
        }
        return $value;
    }
 ...
 public static function amountGross($amount, $givenTax = null)
 {
	 $settings = AdminSettings::first();
     ...
     $taxes = $givenTax === null ? 0 : $givenTax;
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    static public function timeAgo(string|int $timestamp, bool $countdown = false, bool $pluralS = true, array $prefixes = array()): string {
        ...
        if((24 * 365) < $hoursFloor) { //Year in hours
            ...
            $response = $count . ($pluralS ? " " . self::pluralS($count,$prefixes["year"]) : $prefixes["year"]) . " " . $prefixes["ago"];
        }
            ...
            $response = $count . ($pluralS ? " " . self::pluralS($count,$prefixes["month"]) : $prefixes["month"]) . " " . $prefixes["ago"];
        }
        else if(24 < $hoursFloor) { // day in hours
            $count = floor($hoursFloor / 24);
            $response = $count . ($pluralS ? " " . self::pluralS($count,$prefixes["day"]) : $prefixes["day"]) . " " . $prefixes["ago"];
        }
        else  {
            if(!($hoursFloor > 0)) {
                $count = round($difference / 60);
                $response = $count . ($pluralS ? " " . self::pluralS($count,$prefixes["minute"]) : $prefixes["minute"]) . " " . $prefixes["ago"];
            }
            else $response = $hoursFloor . ($pluralS ? " " . self::pluralS($hoursFloor,$prefixes["hour"]) : $prefixes["hour"]) . " " . $prefixes["ago"]; // Hours
        }
