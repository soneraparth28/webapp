...
class TipController extends Controller
{
  ...
  $validator = Validator::make($this->request->all(), [
    ...
    $isLive = (int)$this->request->isLive;

      if($isLive) {
          $live = LiveStreamings::whereId($this->request->liveId)
              ->where('updated_at', '>', now()->subMinutes(5))
              ->whereStatus('0')
              ->firstOrFail();
          $recipientUser = $live->user();
          $recipientId = $recipientUser->id;
      }
      elseif($this->request->recipient_id !== null) {
          $recipientId = $this->request->recipient_id;
          $recipientUser = User::where("id", $recipientId)->firstOrFail();
      }
      else return response()->json([ "success" => false, "errors" => ['error' => "Failed to find recipient user"] ]);
      ...
      switch ($this->request->payment_gateway_tip) {
          ...
          case "card":
              ...
              $details["seller_id"] = $recipientId;
              $details["taxes"] = auth()->user()->isTaxable();
              $details["description"] = $isLive ? "Livestream tip" : "A generous tip";
              $details["metadata"] = [
                  ...
                  "type" => "tip",
                  "is_livestream" => (bool)$isLive,
                  "livestream_id" => $isLive ? $this->request->liveId : 0,
                  "redirect_uri" => $isLive ? url('live/' . $recipientUser->username ) : url($recipientUser->username),
                  "subscription_id" => 0,
              ];
