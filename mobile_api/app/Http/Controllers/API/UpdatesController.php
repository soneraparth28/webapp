...
//            abort(500);
        ...
        $query->when($this->request->input('media') == 'videos', function($q) use($user) {
            $q->where('media.video', '<>', '')->where(function ($query) {
                ...
                })->orWhere('media.video_embed', '<>', '')->where('media.user_id', $user->id);
        });
        //=== Audio
        ...
        $data = $query->orderBy('updates.id','desc')
            ->groupBy('updates.id')->skip($skip)->take($this->settings->number_posts_show)->get();
...
//        return view('users.edit-update')->withData($data);
    ...
    public function ajaxBookmarksUpdates()
    {
        ...
        $response = [
            ...
            'message' => 'Ajax Bookmarks Update Data.',
        ];
        return response()->json($response , 200);
//        return view('includes.updates',
//            ['updates' => $data,
...
//            abort(404);
    ...
    public function getFileMedia($typeMedia, $fileId)
    {
        ...
        if (auth()->check() && auth()->id() == $response->updates->user_id || auth()->check() && $response->updates->locked == 'yes'
            && $checkUserSubscription && $response->updates->price != 0.00 && $checkUserSubscription->free == 'no'
            || auth()->check() && $response->updates->locked == 'yes' && $checkUserSubscription && $response->updates->price == 0.00
            || auth()->check() && auth()->user()->payPerView()->where('updates_id', $response->updates->id)->first()
