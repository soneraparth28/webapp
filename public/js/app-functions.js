...
// Block dragging on images
 ...
 if($(document).find(".custom-dropdown-btn").length) {
     let keySelectors = ["custom-dropdown-btn", "custom-dropdown-parent", "custom-dropdown"];
     $(document).on("click", function (e) {
         e.stopPropagation();

         let el = $(e.target);
         for(let selectorClass of keySelectors) {
             if(el.hasClass(selectorClass) || el.parents("." + selectorClass).length) {
                 customDropdownHandler(true, el);
                 return;
             }
         }

         customDropdownHandler(false);
     });
 }

 function customDropdownHandler(open, el) {
     if(open) {
         let btn, dropdownParent, dropdownMenu;
         if(el.hasClass("custom-dropdown-btn")) {
             btn = el;
             dropdownParent = btn.parents(".custom-dropdown-parent").first();
             dropdownMenu = dropdownParent.find(".custom-dropdown").first();
         }
         else if(el.hasClass("custom-dropdown-parent")) {
             dropdownParent = el;
             dropdownMenu = dropdownParent.find(".custom-dropdown").first();
         }
         else if(el.hasClass("custom-dropdown")) dropdownMenu = el;
         else return false;

         if(!dropdownMenu.hasClass("show-custom-dropdown")) dropdownMenu.addClass("show-custom-dropdown")
         return true;
     }

     let openDropdownMenus = $(document).find(".custom-dropdown.show-custom-dropdown");
     if(!openDropdownMenus.length) return true;
     openDropdownMenus.each(function () { $(this).removeClass("show-custom-dropdown") })
 }


 $(document).on("click", "#showMobileSearchForm", function () {
     let form = $(document).find("#search-creators-form").first(), btn = $(this);
     if(!form.length) return false;
     form = form.first();

     if(form.hasClass("mobileFormHidden")) {
         form.removeClass("mobileFormHidden")
     }
     else {
         form.addClass("mobileFormHidden");
     }
 })

    /**
     * CURRENT DISABLES THE FORM TO SUBMIT.
     * REMOVE ME WHEN THE SEARCH CREATOR PAGE IS DONE
     */
 $(document).on("submit", "#search-creators-form form", function (e) {e.preventDefault()})
 ...
 $('#searchCreatorNavbar').on('keyup',function(e) {
	 ...
	 if (trim($string).length < 3) {
		 $('#dropdownCreators').removeClass('show-custom-dropdown');
		 return true;
	 } else if (e.which == 16
		 || e.which == 17
		 || e.which == 18
     ...
     if(!$('#dropdownCreators').hasClass("show-custom-dropdown")) {
         $('.custom-dropdown-btn').trigger('click');
         $(this).focus();
     }
	 $('#spinnerSearch').show();
				 ...
				 success:  function(response) {
					 if (response.success) {
						window.location.href = response.url;
		 				...
		 				for ($key in response.errors) {
		 					error += '<li class="color-light"><i class="fa fa-times-circle color-light"></i> ' + response.errors[$key] + '</li>';
		 				}
...
function dashboardCharts() {
    $.ajax({
        ...
        success: (res) => {
            console.log(res);
            if("country" in res) setGoogleGeoChart(res.country);
            if("date" in res) setGoogleBarChart(res.date);
        },
        error: (res) => {
...
function barChartFormatTooltips(label, number) {
    return "<p class='font-11 mb-0'>" + label + "</p>" +
        "<p class='font-18 font-weight-bold mb-0 text-nowrap' style='letter-spacing: 1px;'>€" + shortNumbByT(number, true, true) + "</p>";
}

function setGoogleBarChart(res) {
    let chartElement = $("#columnchart_values");
    if(!chartElement.length || empty(res)) return;
    chartElement = chartElement.first().get(0);

    let barStyles = "color: black";
    let collector = [["Element", "", { role: "style" }, {'type': 'string', 'role': 'tooltip', 'p': {'html': true}} ]];
    for(let monthDate in res) {
        let list = res[monthDate];
        let year = monthDate.split("-")[0];
        let monthShort = monthIntToShortText(monthDate.split("-")[1])
        let value = Object.values(list).reduce(function (accumulator, item) {
            return accumulator + (typeof item.amount !== "number" ? parseFloat(item.amount) : item.amount);
        }, 0);
        collector.push([monthShort, value, barStyles, barChartFormatTooltips(monthShort + " " + year, value)]);
    }
    ...
    let options = {
        ...
        tooltip: { isHtml: true },
        vAxis: { gridlines: { count: 12 }, format: "short" },
...
function setGoogleGeoChart(res) {
    let chartElement = $("#regions_div");
    if(!chartElement.length || empty(res)) return;
    chartElement = chartElement.first().get(0);

    let collector = [['Country', 'Popularity']];
    for(let country in res.data) collector.push([country, res.data[country].net_sales_excl_vat]);

    let data = google.visualization.arrayToDataTable(collector);
    let options = {
        // height: 600,
        // chartArea: {width: "100%", left: 50, height: 500, top: 10},
        colorAxis: {colors: ['#F9F9F9', '#000']},
        legend: "none"
    };

    let chart = new google.visualization.GeoChart(chartElement);
