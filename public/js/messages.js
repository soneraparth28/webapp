...
(function($) {
	...
	$(document).on('click','#button-reply-msg',function(s) {
	 ...
	 var replyBtnParent    = element.parents().first();
	 ...
	 if (error == false) {
         element.hide();
         replyBtnParent.append(dataWait);
	 ...
	 (function() {
	   ...
	   var percentVal = '0%';
	    ...
	    error: function(responseText, statusText, xhr, $form) {
            replyBtnParent.find(".spinner-border").first().remove()
            element.show();
	...
	function Chat($idFirstMsg) {
		...
		var param    = /^[0-9]+$/i;
		var _lastId  = $('div.chatlist:last').attr('data');
		var _userId  = $('#contentDIV').attr('data');
	    var _firstMsg = false;
	      ...
	      if (res.userOnline) {
	        ...
	        jQuery(".timeAgo").timeago();
				...
				for (var i = 0; i < total_data; ++i) {
					$( res.messages[i] ).hide().appendTo('#contentDIV').fadeIn(500);
					}

					jQuery(".timeAgo").timeago();

				   	var myDiv = $("#contentDIV").first().get(0);
