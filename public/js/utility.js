...
$(document).on("click", "button#invoiceLookupBtn", function () {
   invoiceLookup()
});
...
function monthIntToShortText(num) {
   switch (num) {
      case 1:
      case "01":
         return "Jan";
...
function drawChart(chartData, metaData) {
      ...
      xaxis: {
         ...
         labels: {
            ...
            formatter: function (value, timestamp) {
               ...
               if (rangeType === "month") {
                  let dayOfWeekName = givenDate.toLocaleString('default', {
                     weekday: 'short'
                  });
                  let dayOfMonth = givenDate.getDate();
                  if (dayOfMonth < 10) dayOfMonth = "0" + dayOfMonth;
                  let monthOfYear = givenDate.toLocaleString('default', {
                     month: 'short'
                  });
                  return [dayOfWeekName, dayOfMonth + " " + monthOfYear];
               }
               if (rangeType === "year") return givenDate.toLocaleString('default', {
                  month: 'short'
               });
               return value;
            },
   ...
   if ($(element).find(".apexcharts-canvas").length) {
      let el = $(element),
         resizeEl = el.siblings(".resize-triggers").first();
...
if ($(document).find("#openMediaItemBox").length) {
   let containerBox = $(document).find("#mediaItemBox").first();

   $(document).on("click", function () {
      if (containerBox.hasClass("itemBoxActive")) containerBox.removeClass("itemBoxActive");
   });

   $(document).on("click", "#mediaItemBox, #openMediaItemBox", function (e) {
      e.stopPropagation();
      let el = $(this);

      if (["openMediaItemBox"].includes(el.attr("id"))) {
         if (containerBox.hasClass("itemBoxActive")) containerBox.removeClass("itemBoxActive");
         else containerBox.addClass("itemBoxActive");
      } else if (!(["mediaItemBox"].includes(el.attr("id")) || el.parents("#mediaItemBox").length)) containerBox.removeClass("itemBoxActive");
   });
}
...
function textMimic(textField) {
   ...
   mimicTargetField.each(function () {
      $(this).text(textField.val())
   });
}
...
function selectPlan(btn) {
   ...
   let currencySymbol = currency === "eur" ? "€" : "dkk";


   let displayPrice = $("#show_price"),
      displayPlan = $("#show_plan_name"),
      payNowBtn = $("[name=pay_addon]").first(),
      form = $("#addonSubscription"),
      intervalField = form.find("#interval").first();
   displayPrice.text(currencySymbol + price);
   displayPlan.text(interval);
   intervalField.val(interval);
   payNowBtn.text("Pay " + currencySymbol + price + " now");

   $(document).find(".select-plan").each(function () {
      $(this).removeClass("selected").find(".btn-inner-text").first().text("Choose plan");
   })
   btn.addClass("selected").find(".btn-inner-text").first().text("Selected plan");
...
if ($(document).find(".select2Multi").length) {
   select2MultiInit();
}


$(document).on("click", "#leftSidebarOpenBtn", function () {
   $(document).find(".session-main-left-menu").addClass("mb-open");
   $(document).find(".session-main-page-wrapper").first().addClass("overlay-blur-dark-small-screen");
})
$(document).on("click", "#leftSidebarCloseBtn", function () {
   $(document).find(".session-main-left-menu").removeClass("mb-open");
   $(document).find(".session-main-page-wrapper").first().removeClass("overlay-blur-dark-small-screen");
})


if ($(document).find("#page-dashboard").length) {
   google.charts.load("current", {
      packages: ['corechart', "geochart"]
   });
   google.charts.setOnLoadCallback(dashboardCharts);
}