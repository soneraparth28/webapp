...
<div class="media-body position-relative">
    ...
    @if($mediaCount > 0)
        <div class="@if ($mediaCount == 0) float-right @else wrapper-msg-left @endif message position-relative text-word-break @if ($mediaCount == 0 && $msg->tip == 'no') bg-light-gray @else media-container @endif color-dark @if ($msg->format == 'zip') w-50 @else w-auto @endif  rounded-bottom-right-0">
  ...
  @if ($msg->message != '')
      <div class="flex-row-end flex-align-center flex-nowrap">
          <div class="position-relative text-word-break message  color-dark border-radius-10px" style="background: #E9E9E9;">
              {!! $chatMessage !!}
          </div>

          @if ($msg->tip == 'no')
              <a href="javascript:void(0);" class="ml-2 removeMsg" data="{{$msg->id}}" title="{{trans('general.delete')}}">
                  <i class="bi bi-x"></i>
              </a>
          @endif
      </div>
...
@if ($mediaCount > 0 && $msg->price == 0.00 || $msg->price != 0.00 && $checkPayPerView)
  ...
  @if ($msg->message != '')
    <div class="w-100 d-inline-block">
      <div class="w-auto position-relative text-word-break message bg-light float-left border-radius-10px">
        {!! $chatMessage !!}
      </div>
  </div>
@endif
