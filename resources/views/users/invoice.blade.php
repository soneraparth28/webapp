  ...
  <section class="invoice">
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
        {{trans('general.to')}}
        <address>
          <strong>{{$data->invoice_name}} {{!blank($data->invoice_company) ? ' - '.$data->invoice_company : "" }}</strong><br>

          @if ($data->invoice_address)
            {{$data->invoice_address}}<br>
          @endif

          @if ($data->invoice_city || $data->invoice_zip)
            {{$data->invoice_city}}, {{$data->invoice_zip}}<br>
          @endif

          {{$data->invoice_country}} <br>
          {{trans('auth.email')}}: {{$data->invoice_email}}
        </address>
      </div>
      <!-- /.col -->
      <div class="col-sm-4 invoice-col">
